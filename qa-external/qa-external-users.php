...
/**
 * ==========================================================================================================
 * THIS FUNCTIONS WILL CONNECT TO THANKYO DATABASE AND SAVE THE CONNECTION FOR OTHER QUERIES
 * ==========================================================================================================
 *
 * streamkar_connect_to_thankyo_db()
 * 
 * This function makes a connection with Thankyo database
 *
 */
 
 $streamkar_con = null;

function streamkar_connect_to_thankyo_db(){
	global $streamkar_con;	
	if($streamkar_con == null){
		$streamkar_con = mysqli_connect(THANKYO_MYSQL_HOSTNAME,THANKYO_MYSQL_USERNAME,THANKYO_MYSQL_PASSWORD,THANKYO_MYSQL_DATABASE);
   
		if (mysqli_connect_errno($streamkar_con)){
			echo "Failed to connect to MySQL: " . mysqli_connect_error();
		}
	}
}

function streamkar_close_thankyo_db(){
	global $streamkar_con;
		
		if($streamkar_con != null)
			mysqli_close($streamkar_con);
}

function streamkar_is_user_admin($username){
	global $streamkar_con;
	...
	$query="SELECT * FROM t_manageinfo WHERE accountname=".$username;
	$result=mysqli_query($streamkar_con, $query);
...
function qa_get_login_links($relative_url_prefix, $redirect_back_to_url)
{
	...
	return array(
		'login' => 'http://localhost:8089/?action=login&redirect='.urlencode('http://localhost:8085/git_questions_and_answers/'.$redirect_back_to_url),
		'register' => 'http://localhost:8089/?action=register&redirect='.urlencode('http://localhost:8085/git_questions_and_answers/'.$redirect_back_to_url),
		'logout' => 'http://localhost:8089/?action=logout&redirect='.urlencode('http://localhost:8085/git_questions_and_answers/'.$redirect_back_to_url),
	);
...
{
	//$login_info = THANKYO_LOGIN_INFO);
	//echo file_get_contents(THANKYO_LOGIN_INFO);
	
	/*$json_a = json_decode($login_info, true);   //When TRUE, returned objects will be converted into associative arrays.
	...
	//if (isset($_COOKIE['userid']) && isset($_COOKIE['publicusername']) && isset($_COOKIE['email'])) {
				 
				return array(
					'userid' => 2000311,//$_COOKIE['userid'],
					'publicusername' => "wj",//$_COOKIE['publicusername'],
					'email' => "jiang@thankyo.net",//$_COOKIE['email'],
					'level' =>  QA_USER_LEVEL_ADMIN
					//'level' => (streamkar_is_user_admin($_COOKIE['publicusername']) == true) ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC 
				);
...
function qa_get_user_email($userid)
{
	...
	$query="SELECT email FROM t_user WHERE id=".$userid;
	$result=mysqli_query($streamkar_con, $query);
	...
	$publictouserid = array();
		...
		if (count($publicusernames)) {
			...
			foreach ($publicusernames as $publicusername)
				...
				$query="SELECT id,account FROM t_user WHERE account IN (" . implode(',', $escapedusernames) . ")";
				$result=mysqli_query($streamkar_con, $query);
				
				$results = qa_db_read_all_assoc($result);
				
			foreach ($results as $result)
				$publictouserid[$result['account']] = $result['id'];
		}
	...
	$useridtopublic = array();
		...
		if (count($userids)) {
			...
			foreach ($userids as $userid)
				...
				$query="SELECT id,account FROM t_user WHERE id IN (" . implode(',', $escapeduserids) . ")";
				$result=mysqli_query($streamkar_con, $query);
				
				$results = qa_db_read_all_assoc($result);
				foreach ($results as $result1){
					$useridtopublic[$result1['id']] = $result1['account'];
				}
		}
...
function qa_avatar_html_from_userid($userid, $size, $padding)
{
	...
	$query="SELECT headimg,status FROM t_user WHERE id =" . $userid;
	$result=mysqli_query($streamkar_con, $query);
	
	
	$htmlsize = (int)$size;
	
	$result = qa_db_read_one_assoc($result);

		if (is_array($result)){
			

			
			//show the default image if user has no Avatar
			if($result['headimg'] == null){	
				return '<img src="http://localhost:8085/git_questions_and_answers/img/claim.png" ' . 'width="' . $htmlsize . '" height="' . $htmlsize . '" class="qa-avatar-image" alt=""/>';
				
			}else{
				
			$image_details = explode(".", $result['headimg']);			
			
			//check type of user for determining the type included in S3 URL
			if($result['status'] == 0) 
				$user_type = "8";
			else if($result['status'] == 1)
				$user_type = "7";
				
			// parse userid to make it in pattern of 0000/0200/0408 
			// make userid to be 12 digits 
			$padded_userid=  str_pad($userid, 12, "0", STR_PAD_LEFT);		
			$userid_for_url = substr($padded_userid,0, 4) . "/" . substr($padded_userid,4, 4). "/" . substr($padded_userid,8, 4);			

			return '<img src="'.THANKYO_AVATAR_LINK.$user_type.'/'.$userid_for_url.'/'.$image_details[0].'_256.'.$image_details[1].'" ' .
